    ...
    window.openKeyBackup = async function () {
        ...
        card.innerHTML = eIco(`
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
                <h2 style="margin:0; font-size:17px; color:#2b2926;">🔐 Záloha šifrovacího klíče</h2>
                <button id="kb-close" style="border:none; background:#edeae4; border-radius:8px; width:30px; height:30px; cursor:pointer; font-size:16px;">✕</button>
            </div>
            <div style="background:#f7f0e6; border:1px solid #fed7aa; color:#9a3412; border-radius:10px; padding:12px; font-size:12px; line-height:1.5; margin:10px 0 16px;">
            ...
            <div style="font-size:12px; color:#4a453f; margin-bottom:16px;">
                Stav klíče: ${st.exists ? '✅ existuje' : '⚠️ zatím neexistuje (spusťte LexisLocal)'}
                ${st.fingerprint ? `<br><span style="color:#77716a;">otisk: ${st.fingerprint}</span>` : ''}
            </div>
            <div style="display:flex; flex-direction:column; gap:10px;">
                <button id="kb-backup" style="padding:11px 16px; border:none; background:#9a5b22; color:#fff; border-radius:8px; cursor:pointer; font-size:13px; font-weight:700;">Zálohovat klíč do souboru…</button>
                <button id="kb-restore" style="padding:11px 16px; border:1px solid #ddd6cb; background:#fff; color:#4a453f; border-radius:8px; cursor:pointer; font-size:13px; font-weight:600;">Obnovit klíč ze zálohy…</button>
            </div>
        ...
        const setResult = (html, ok) => { card.querySelector('#kb-result').innerHTML = eIco(`<span style="color:${ok ? '#5a8a4a' : '#b04331'};">${html}</span>`); };
