    ...
    window.openIsdsSettings = async function () {
        ...
        card.innerHTML = eIco(`
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
                <h2 style="margin:0; font-size:17px; color:#2b2926;">📨 Nastavení datové schránky</h2>
                <button id="is-close" style="border:none; background:#edeae4; border-radius:8px; width:30px; height:30px; cursor:pointer; font-size:16px;">✕</button>
            </div>
            <p style="margin:0 0 16px; font-size:12px; color:#77716a;">Připojení k ISDS. „Ověřit spojení" nic neodesílá — jen zkontroluje přihlášení.</p>

            <label style="display:block; font-size:11px; font-weight:700; color:#4a453f; margin-bottom:4px;">Prostředí</label>
            <select id="is-env" style="width:100%; box-sizing:border-box; padding:9px; margin-bottom:14px; border:1px solid #ddd6cb; border-radius:8px; font-size:13px;">
            ...
            <div style="border:1px solid #e0dbd3; border-radius:10px; padding:12px; margin-bottom:16px;">
                <div style="font-size:12px; font-weight:700; color:#4a453f; margin-bottom:8px;">Volitelně: přihlášení certifikátem (.p12)</div>
                <div style="display:flex; gap:8px; align-items:center; margin-bottom:8px;">
                    <button id="is-cert-pick" style="padding:8px 12px; background:#edeae4; color:#4a453f; border:1px solid #ddd6cb; border-radius:8px; cursor:pointer; font-size:12px; font-weight:600;">Vybrat .p12…</button>
                    <button id="is-cert-clear" style="padding:8px 10px; background:#fff; color:#77716a; border:1px solid #e0dbd3; border-radius:8px; cursor:pointer; font-size:12px;">Odebrat</button>
                    <span id="is-cert-path" style="font-size:11px; color:#77716a; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${certPath ? esc(certPath) : 'nevybráno'}</span>
                </div>
                <input id="is-cert-pass" type="password" placeholder="Heslo k certifikátu" style="width:100%; box-sizing:border-box; padding:9px; border:1px solid #ddd6cb; border-radius:8px; font-size:13px;">
            </div>

            <div id="is-result" style="font-size:12px; margin-bottom:12px; min-height:16px;"></div>

            <div style="display:flex; justify-content:space-between; gap:8px;">
                <button id="is-test" style="padding:10px 14px; border:1px solid #9a5b22; background:#fff; color:#9a5b22; border-radius:8px; cursor:pointer; font-size:13px; font-weight:700;">Ověřit spojení</button>
                <div style="display:flex; gap:8px;">
                    <button id="is-cancel" style="padding:10px 14px; border:1px solid #ddd6cb; background:#fff; color:#5c574f; border-radius:8px; cursor:pointer; font-size:13px;">Zrušit</button>
                    <button id="is-save" style="padding:10px 16px; border:none; background:#9a5b22; color:#fff; border-radius:8px; cursor:pointer; font-size:13px; font-weight:700;">Uložit</button>
                </div>
            </div>`);
        ov.appendChild(card);
        ...
        const setResult = (html, ok) => { $('#is-result').innerHTML = eIco(`<span style="color:${ok ? '#5a8a4a' : '#b04331'};">${html}</span>`); };
